...
class ViewModel {
    ...
    private func fetchAccessTokenDetails() async throws -> AccessTokenDetails {
        guard let url = URL(string: "\(Secrets.Sandbox.audience)/v1/oauth/token") else {
    ...
    private func verifyFundsAndCreateTransfer(
        ...
        do {
            guard let url = URL(string: "\(Secrets.Sandbox.audience)/v1/quote/\(transferRequest.quoteHistoryId)") else {
                throw URLError(.badURL)
            }
            ...
            guard let url = URL(string: "\(Secrets.Sandbox.audience)/v1/transfers") else {
                throw URLError(.badURL)
            }
